...
export default function DocumentViewPage() {
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        // Expecting UserProfile directly, not wrapped in ApiResponse
        const userProfile = await ApiService.get('/auth/profile') as UserProfile;
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-base">{document.tipoDocumento || 'N/A'}</p>
                </div>
                <div>
                ...
                {document.valor && (
                  <div>
                    ...
                    <p className="text-base">{formatCurrency(document.valor)}</p>
                  </div>
                )}
                ...
                {document.fornecedor && (
                  <div>
                    ...
                    <p className="text-base">{document.fornecedor.name}</p>
                  </div>
                )}
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Tamanho do Arquivo</p>
                  <p className="text-base">{formatBytes(document.fileSize)}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Hash do Arquivo</p>
                  <p className="text-base break-all font-mono text-xs">{document.fileHash}</p>
                </div>
                      ...
                      <TableBody>
                        {signatories.sort((a, b) => a.order - b.order).map((sig) => (
                          <TableRow key={sig.id} className={sig.user.id === currentUserId ? 'bg-blue-50' : ''}>
                            ...
                            <TableCell>{getSignatureStatusBadge(sig.status)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between text-sm text-muted-foreground">
                      ...
                      <span className="font-semibold">{formatCurrency(calculateAllocations(document.allocations, document.valor).totalValor)}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm text-muted-foreground">
